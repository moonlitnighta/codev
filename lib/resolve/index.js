 ...
 */
async function findPlatformName(filePath, mainFields) {
  const pathObj = path.parse(filePath)
  const list = await utils.readdir(pathObj.dir)
  if (list && list.length) {
    const str = '/' + list.join('/') + '/'
    ...
    for (let i = 0; i < mainFields.length; i ++) {
      ...
      if (match) {
        return utils.isPath(path.resolve(pathObj.dir, fileName + match[1] + platform + pathObj.ext))
      }
    }
  } else {
    return 'no-file'
 ...
 */
async function nodeModulesFind(packagePath, _path) {
      ...
      if (utils.isObject(pack.browser)) {
        for (let k in pack.browser) {
          if (path.resolve(context, k) === path.resolve(context, _path)) {
            return await utils.isPath(path.resolve(context, pack.browser[k]))
          }
        }
      }
    }
 ...
 */
async function findPackFile(resolve, context, moduleName){
  let filePath = false;

  // 利用缓存node_modules目录查找一次
  if (nodeModulesPath) {
    filePath = await findPackage(resolve, path.resolve(nodeModulesPath, moduleName, 'package.json'))
  ...
  for(let i = tables.length; i --;){
    if(tables[i]){
      filePath = await findPackage(resolve, path.resolve(_context, packagePath))
      if(filePath){
        // 缓存node_modules目录
 ...
 */
async function patchSuffixFind(extensions, context) {
  let filePath
  for (let i = 0; i < extensions.length; i ++) {
    filePath = await utils.isPath(context + extensions[i])
    if (filePath) { return filePath }
  }
}
 ...
 * @return {string}             文件有效路径
 */
async function findPackage(resolve, packagePath) {
  let pack = null
  let filePath
  try {
    pack = require(packagePath)
    if (pack) {
      for (let i = 0; i < resolve.mainFields.length; i ++) {
        ...
        if (filePath) {
          ...
          // 是个目录还是文件
          const isList = await utils.isList(context)
          
          if (!isList) {
            // 有些大神连后缀都不写...
            if (suffix) {
              const file = await utils.isPath(context)
              if (file) return file

              // 后缀还是个假的？ xxx.module === xxx.module.js 
              return await againstFind(path.dirname(packagePath), filePath , resolve)
            } else {
              return await againstFind(path.dirname(packagePath), filePath , resolve)
            }
          } else {
      ...
      return await patchSuffixFind(extensions, context)
    }
  } catch (err) {
    return false
  }
 ...
 * @return {string}          文件有效路径
 */
async function contextFind(resolve, context, filePath) {
  let _path = ''
  // 如果指定node_modules路径则优先查找
  if (resolve.modulesPath) {
    _path = await findPackage(resolve, `${resolve.modulesPath}/${filePath}/package.json`)
  ...
  const extensions = resolve.priority
  for (let i = 0; i < extensions.length; i ++) {
    _path = await utils.isPath(path.resolve(context , filePath + extensions[i]))
    if (_path) { return _path }
  }
...
* @return {object}          文件信息
**/
async function againstFind(context , filePath , resolve){
  let _path
  let dir = path.extname(filePath)

  // 如果有后缀则尝试直接使用当前上下文目录查找一次
  if (dir) {
    _path = await utils.isPath(path.resolve(context , filePath))
    if (_path) return _path
  }
  ...
  if (isAbsolute) {
    for (let i = 0; i < extensions.length; i ++) {
      for (let l = 0; l < fileName.length; l ++) {
        _path = await utils.isPath(fp + fileName[l] + extensions[i])
        if (_path) return _path
      }
  ...
  for(let i = tables.length; i --;){
    for (let n = 0; n < extensions.length; n ++){
      for (let l = 0; l < fileName.length; l ++) {
        ...
        _path = await findPlatformName(p, resolve.mainFields)

        //----------------------------------------------------------------------------------------------------
        
        // 补全路径在项目文件下查找一次
        if (!_path) {
          _path = await utils.isPath(p)
          // 'adsads.asd.js'这样???????
          if (!_path && suffix) {
            _path = await patchSuffixFind(resolve.priority, p)
          }
        ...
        if (!_path && !relatively) {
          ...
          if (packName && fn) {
            ...
            _path = await nodeModulesFind(modulesPath ? path.resolve(modulesPath , packName) : '', fn) ||
                    await nodeModulesFind(path.resolve(nodeModulesPath ,packName), fn) ||
                    await nodeModulesFind(path.resolve(_context , 'node_modules', packName), fn)
          }
        ...
        if (!_path && !relatively) {
          _path = await utils.isPath(modulesPath ? path.resolve(modulesPath , url) : '') ||
                  await utils.isPath(path.resolve(nodeModulesPath, url)) ||
                  await utils.isPath(path.resolve(_context , 'node_modules/' + url))
 ...
 */
module.exports = async function (options, context, filePath){
  ...
  if (!url) {
    ...
    if(filePath.indexOf('/') > -1 || path.extname(filePath)){
      file = await againstFind(context , filePath , resolve)

      // 难道是'xxx/'或'xxx.xx'并且是引入第三方包 ？？
      if (!file) {
        file = await findPackFile(resolve , context , filePath) 
      }
    } else {
      file = await contextFind(resolve, context, filePath) || await findPackFile(resolve , context , filePath)
    }
  } else {
    file = await utils.isPath(url)
  }

  // 找不到模块直接退出程序
  if (!file) {
    utils.error(`module not find in ${filePath}`)
    utils.exit()
  }
  ...
  const source = await utils.readFile(uri)
