...
const codevExclude = new RegExp(CONSTANT.EXCLUDE)
...
/**
 * 根据头部规则排除文件
 * @param  {object} module      校验的模块
 */
function headCheck(module) {
  return codevExclude.test(module.source.toString())
} 
...
async function loader(depTree , options , parentModule , filePath) {
  ...
  const module = utils.isNetwork(filePath) || utils.isNetwork(parentPath) ? 
                 ...
                 resolve(options, context, filePath)
  ...
  // 根据头部注释规则排除文件
  if (options.resolve.enableExclusion && headCheck(module)) {
    return null
  }
